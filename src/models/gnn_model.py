...
class CardRecommendationGNN(nn.Module):
    """
    Graph Neural Network for Clash Royale card recommendation.
    
    Takes a graph with all cards as nodes and recommends 2 cards given 6 input cards.
    Uses GCN layers with Xavier weight initialization.
    """
    ...
    def __init__(
        ...
        hidden_dims: list,
        dropout_rates: list,
        gnn_type: str = "GCN",  # Kept for backward compatibility
        num_cards: int = 110,
        weight_init: str = "xavier"  # Kept for backward compatibility
    ):
        """
        ...
        Args:
            ...
            gnn_type: Type of GNN layer (default: "GCN", kept for compatibility)
            num_cards: Total number of cards (for output layer)
            weight_init: Weight initialization method (default: "xavier", kept for compatibility)
        """
        super(CardRecommendationGNN, self).__init__()
    ...
    def _init_weights(self):
        ...
        GNN layers use smaller std for output layer stability.
        """
        # Initialize GNN layers with Xavier
        for gnn_layer in self.gnn_layers:
            if hasattr(gnn_layer, 'lin') and gnn_layer.lin is not None:
                nn.init.xavier_uniform_(gnn_layer.lin.weight)
                if gnn_layer.lin.bias is not None:
                    nn.init.constant_(gnn_layer.lin.bias, 0)
