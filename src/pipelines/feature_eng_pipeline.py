...
def create_training_examples(
    ...
    margin_weights: Optional[Dict[int, float]] = None
) -> List[Dict]:
    """
    Create training examples from decks with optional win/loss weighting.
    For each deck of 8 cards, create examples with 6 input cards and 2 target cards.
    
    Args:
        decks: List of deck dictionaries with card IDs and optional crown_margin
        id_to_index: Mapping from card ID to node index
        index_to_id: Mapping from node index to card ID
        margin_weights: Optional mapping from crown_margin (-3 to 3) to sample weight.
                       If None, all examples have weight 1.0.
                       Default weights prioritize winning decks.
        
    Returns:
        List of training examples with weights
    """
    # Default margin weights if not provided
    if margin_weights is None:
        margin_weights = {
            3: 1.0,    # 3-crown win (dominant)
            2: 0.8,    # 2-crown win
            1: 0.6,    # 1-crown win
            0: 0.0,    # Draw (skip by default)
            -1: 0.2,   # 1-crown loss
            -2: 0.1,   # 2-crown loss
            -3: 0.05,  # 3-crown loss
        }
    ...
    for deck in decks:
        ...
        # Get crown margin and compute weight
        crown_margin = deck.get("crown_margin", None)
        
        if crown_margin is not None:
            weight = margin_weights.get(crown_margin, 0.5)
            # Skip if weight is 0 (typically draws)
            if weight == 0:
                skipped_draws += 1
                continue
        else:
            # Legacy data without crown_margin: use default weight
            weight = 1.0
            skipped_no_margin += 1
    ...
    if skipped_draws > 0:
        print(f"  Skipped {skipped_draws} draws (weight=0)")
    if skipped_no_margin > 0:
        print(f"  Note: {skipped_no_margin} decks without crown_margin (using default weight=1.0)")
    
    # Weight distribution
    if examples:
        weights = [e["weight"] for e in examples]
        print(f"  Weight distribution: min={min(weights):.2f}, max={max(weights):.2f}, avg={sum(weights)/len(weights):.2f}")
...
def process_features(
    ...
    # Create training examples with margin weights
    print("Creating training examples...")
    margin_weights = config.get("training", {}).get("margin_weights", None)
    if margin_weights:
        # Convert string keys to int (JSON keys are always strings)
        margin_weights = {int(k): float(v) for k, v in margin_weights.items()}
        print(f"Using margin weights from config: {margin_weights}")
    training_examples = create_training_examples(decks, id_to_index, index_to_id, margin_weights)
